...
export class CartsService {
  ...
  public async getCart(userId: string): Promise<CartModel>{
    ...
    if (!cart) {
      ...
      store.dispatch(getCartAction(cart)); //need to fix this shit
    }
    return cart
  }
  ...
  // public async createCart(cart: CartModel): Promise<CartModel>{
  //   const addedCart = await firstValueFrom(this.http.post<CartModel>(environment.cartByUserUrl, cart));
  //   return addedCart
  // }
